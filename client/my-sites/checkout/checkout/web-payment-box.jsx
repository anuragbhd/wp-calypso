...
function LoadingPaymentRequestButton( { translate } ) {
	return <button disabled>{ translate( 'Loading' ) }</button>;
}
...
function getPaymentRequestTotalFromCart( cart, translate ) {
	return {
		...
		amount: cart.total_cost ? cart.total_cost * 100 : 0, // TODO: convert this to the lowest integer value in the currency
	};
}

function getDisplayItems( { translate, sub_total, total_tax } ) {
	return [
		{
			label: translate( 'Subtotal' ),
			amount: sub_total, // TODO: convert to smallest integer in currency
		},
		{
			label: translate( 'Tax', { comment: 'The tax amount line-item in payment request' } ),
			amount: total_tax, // TODO: convert to smallest integer in currency
		},
	];
}
